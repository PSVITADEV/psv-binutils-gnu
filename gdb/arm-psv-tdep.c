...
static bool find_vitalink_section(bfd *abfd, asection *section, void *)
{
...
arm_vita_osabi_sniffer (bfd *abfd)
{
    ...
    if (bfd_sections_find_if(abfd, find_vitalink_section, NULL) != NULL)
        return GDB_OSABI_PSVITA;
    else
        return GDB_OSABI_UNKNOWN;
}

INIT_GDB_FILE(arm_psv_tdep)
{
    gdbarch_register_osabi_sniffer (bfd_arch_arm, bfd_target_elf_flavour,
                                   arm_vita_osabi_sniffer);
    gdbarch_register_osabi (bfd_arch_arm, 0, GDB_OSABI_PSVITA,
                            arm_vita_init_abi);
}
